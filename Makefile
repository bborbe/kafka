...
format:
	...
	find . -type f -name '*.go' -not -path './vendor/*' -exec go run -mod=mod github.com/incu6us/goimports-reviser -project-name github.com/bborbe/kafka -file-path "{}" \;
...
test:
	go test -mod=mod -p=$${GO_TEST_PARALLEL:-1} -cover -race -coverprofile=coverage.out $(shell go list -mod=mod ./... | grep -v /vendor/)
